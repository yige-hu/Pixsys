...
#define NV_REMAP_PAGE_RANGE(v, start, offset, x...) \
    remap_pfn_range(v, start, ((offset) >> PAGE_SHIFT), x)
#elif defined(NV_REMAP_PAGE_RANGE_PRESENT)
#if (NV_REMAP_PAGE_RANGE_ARGUMENT_COUNT == 5)
#define NV_REMAP_PAGE_RANGE(v, x...) remap_page_range(v, x)
#elif (NV_REMAP_PAGE_RANGE_ARGUMENT_COUNT == 4)
#define NV_REMAP_PAGE_RANGE(v, x...) remap_page_range(x)
#else
...
#endif
#if !defined(NV_XEN_SUPPORT_FULLY_VIRTUALIZED_KERNEL)
#define NV_IO_REMAP_PAGE_RANGE(v, offset, size, extra_prot) \
    NV_REMAP_PAGE_RANGE(v, v->vm_start, offset, size,       \
        __pgprot(pgprot_val(v->vm_page_prot) | extra_prot))
#else
#define NV_IO_REMAP_PAGE_RANGE(v, offset, size, extra_prot)             \
    io_remap_pfn_range(v, v->vm_start, ((offset) >> PAGE_SHIFT), size,  \
        __pgprot(pgprot_val(v->vm_page_prot) | extra_prot))
#endif
